...
function libraryHashPlaceholder(fullyQualifiedLibraryName) {
    return "$".concat((0, js_sha3_1.keccak256)(fullyQualifiedLibraryName).slice(0, 34), "$");
}
/**
...
function replacePlaceholder(bytecode, label, address) {
    ...
    var truncatedName = label.slice(0, 36);
    var libLabel = "__".concat(truncatedName.padEnd(36, '_'), "__");
...
function linkBytecode(bytecode, libraries) {
    ...
    for (var _i = 0, _a = Object.entries(libraries); _i < _a.length; _i++) {
        var _b = _a[_i], fullyQualifiedLibraryName = _b[0], libraryObjectOrAddress = _b[1];
        if ((0, helpers_1.isNil)(libraryObjectOrAddress)) {
            throw new Error("No address provided for library ".concat(fullyQualifiedLibraryName));
        }
        // API compatible with the standard JSON i/o
        // {"lib.sol": {"L": "0x..."}}
        if ((0, helpers_1.isObject)(libraryObjectOrAddress)) {
            for (var _c = 0, _d = Object.entries(libraryObjectOrAddress); _c < _d.length; _c++) {
                var _e = _d[_c], unqualifiedLibraryName = _e[0], address = _e[1];
                librariesComplete[unqualifiedLibraryName] = address;
                librariesComplete["".concat(fullyQualifiedLibraryName, ":").concat(unqualifiedLibraryName)] = address;
            }
            continue;
        }
        // backwards compatible API for early solc-js versions
        var parsed = fullyQualifiedLibraryName.match(/^(?<sourceUnitName>[^:]+):(?<unqualifiedLibraryName>.+)$/);
        var libraryAddress = libraryObjectOrAddress;
        if (!(0, helpers_1.isNil)(parsed)) {
            var unqualifiedLibraryName = parsed.groups.unqualifiedLibraryName;
            librariesComplete[unqualifiedLibraryName] = libraryAddress;
        }
        librariesComplete[fullyQualifiedLibraryName] = libraryAddress;
    }
    for (var libraryName in librariesComplete) {
        var hexAddress = librariesComplete[libraryName];
        if (!hexAddress.startsWith('0x') || hexAddress.length > 42) {
            throw new Error("Invalid address specified for ".concat(libraryName));
        }
        // remove 0x prefix
